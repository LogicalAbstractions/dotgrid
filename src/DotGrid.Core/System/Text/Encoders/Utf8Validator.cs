...
namespace System.Buffers.Text
{
    ...
    public struct Utf8Validator
    {
        ...
        private static unsafe uint ConsumeDataWithExistingPartialSequence(uint partialSequence, ReadOnlySpan<byte> utf8Bytes)
        {
            ...
            var bytes = stackalloc byte[sizeof(uint)];
            // Copy as much data as we can from the input buffer to our partial sequence.
            Span<byte> partialSequenceAsBytes = new Span<byte>(bytes,sizeof(uint));
        ...
        private static unsafe uint ConsumeDataWithoutExistingPartialSequence(ReadOnlySpan<byte> utf8Bytes)
        {
