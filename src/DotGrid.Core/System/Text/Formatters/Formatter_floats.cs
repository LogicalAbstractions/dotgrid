...
namespace System.Buffers.Text
{
    ...
    {
        private static unsafe bool TryFormatNumber(double value, bool isSingle, Span<byte> buffer, out int bytesWritten, StandardFormat format = default, SymbolTable symbolTable = null)
        {
            if (!(format.Symbol == 'G' || format.Symbol == 'E' || format.Symbol == 'F'))
            {
            ...
            else
            {
                ...
                var tempBytes = stackalloc byte[needed];
                Span<byte> temp = new Span<byte>(tempBytes,needed);
