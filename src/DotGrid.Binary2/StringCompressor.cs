...
{
    internal unsafe class SmallStringCompression
    {
        ...
        internal SmallStringCompression(string[] termsTable)
        {
        ...
        internal int Decompress(byte* input, int inputLen, byte* output, int outputLen)
        {
            var outPos = 0;
            for (int i = 0; i < inputLen; i++)
            {
                ...
                else
                {
                    ...
                    if (outPos + len > outputLen)
                        return 0;
                    Copy(output, _termsTableBytes[slot] + 1, len);
                    output += len;
                    outPos += len;
                }
            }
        ...
        internal int Compress(byte* input, byte* output, int inputLen, int outputLen)
        {
            ...
            for (int i = 0; i < inputLen; i++)
            {
                ...
                for (; size > 0 && foundMatch == false; size--)
                {
                    ...
                    for (int j = 0; j < slot.Length; j++)
                    {
                        ...
                        if (termLegnth != size ||
                            Compare(input + i, slot[j] + 1, size) != 0)
                        {
                            continue;
                        }
        ...
        private void Flush(byte* input, byte* output, CompressionState* state)
        {
            ...
            while (verbatimLength > 0)
            {
                ...
                Copy(output + state->OutputPosition, input + state->VerbatimStart, len);
        ...
        private void Copy(byte* target, byte* source, int length)
        {
        ...
        private static int Compare(void* p1, void* p2, int size)
        {
